...
class Track:
    ...
    def update_frames(self, node):
        ...
        for frame in range(start_frame, mid_frame):
            frame_box = start_box * (mid_frame - frame) / (mid_frame - start_frame) + mid_box * (
                        frame - start_frame) / (mid_frame - start_frame)
        ...
        for frame in range(mid_frame + 1, end_frame + 1):
            frame_box = mid_box * (end_frame - frame) / (end_frame - mid_frame) + end_box * (frame - mid_frame) / (
                        end_frame - mid_frame)
            if frame not in self.frames:
                self.frames[frame] = [frame_box, 1, score]
            else:
...
def update_tracks(tracks, tube, arg):
    ...
    for idx, track in enumerate(tracks):
        ...
        if np.sum(has_frame) == 0:  # tube and track does not overlap
            continue
        ...
        if np.linalg.norm(track_direction) * np.linalg.norm(tube_direction) > 0:
            angle_cos = np.dot(track_direction, tube_direction) / (
                        np.linalg.norm(track_direction) * np.linalg.norm(tube_direction))
        else:
            angle_cos = 1
        ...
        iou[0] = np.sum(bbox_iou(frame_boxs, track_boxs)[has_frame])
        iou[1] = np.sum(has_frame)
...
def matching(tubes, arg, save_path=None, verbose=False, mid_only=True):
    """
